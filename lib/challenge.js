...
// You have received an array of objects representing data about the women competing for Kolton's heart. Follow the prompts
// to job Kolton's memory using array methods.
...
for (let i = 0; i < contestants.length; i ++) {
  let currentObj =contestants[i];
  if (currentObj.state === 'CA') {
    cali.push(currentObj.firstName);
  }
}
...
for (let j = 0; j < contestants.length; j ++) {
  let currentObj =contestants[j];
  if (currentObj.state == 'CA' && currentObj.zodiacSign == 'Aries') {
    caliAries.push(currentObj.firstName);
  }
}
console.log(caliAries.length);
...
for (let l = 0; l < contestants.length; l++) {
  let curObj = contestants[l];
  curObj.icebreaker = `Hi ${curObj.firstName}, what do you like the most about ${curObj.city}?`;
  //console.log(curObj.icebreaker);
  newArr.push(curObj);
}
console.log(newArr);
...
for (let k = 0; k < contestants.length; k ++) {
  let currentObj =contestants[k];
  if (currentObj.state === 'MI') {
    michigan.push(currentObj.firstName);
  }
}
if (michigan.length >= 1) {
  console.log(true);
} else {
  console.log(false);
}
//console.log(michigan.length);
...
for (let m = 0; m < contestants.length; m ++) {
  let currentObj = contestants[m];
  if (currentObj.zodiacSign === 'Aries') {
    console.log(`Hi, my name is ${currentObj.firstName} ${currentObj.lastInitial} from ${currentObj.city}, and I was the first to sign up!`);
    break;
  } else {
